...
(() => {
  ...
  (function() {
    ...
    const API_AUTH_NOTICE_THROTTLE_MS = 15e3;
    const API_REQUEST_TIMEOUT_MS = 12e3;
    ...
    const UPDATE_CHECK_TTL_MS = 60 * 60 * 1e3;
    const UPDATE_AUTO_CHECK_INTERVAL_MS = 15 * 60 * 1e3;
    ...
    const BYE_PLACEHOLDER_TOKENS = /* @__PURE__ */ new Set([
      "bye",
      "freilos",
      "tbd",
      "tobeconfirmed",
    ...
    function normalizeLookup(value) {
      return normalizeText(value).toLowerCase().normalize("NFKD").replace(/[\u0300-\u036f]/g, "");
    }
    ...
    function cloneDebugValue(value, fallbackValue = null) {
      ...
      if (cloned === null && value !== null && value !== void 0) {
    ...
    function normalizeMatchStartDebugSession(rawSession) {
      ...
      const steps = Array.isArray(session.steps) ? session.steps.map((entry) => normalizeMatchStartDebugStep(entry)).slice(-MATCH_START_DEBUG_STEP_LIMIT) : [];
      return {
    ...
    function normalizeDebugData(rawDebugData) {
      ...
      const matchStartSessions = Array.isArray(debugData.matchStartSessions) ? debugData.matchStartSessions.map((entry) => normalizeMatchStartDebugSession(entry)).slice(-MATCH_START_DEBUG_SESSION_LIMIT) : [];
      return {
        matchStartSessions
      };
    }
    ...
    function serializeMatchStartError(error, extractErrorText = null) {
      const fallbackText = typeof extractErrorText === "function" ? normalizeText(extractErrorText(error)) : "";
    ...
    function shouldRetryLobbyCreateWithBullModeFallback(error, payload, extractErrorText = null) {
      ...
      const fallbackText = typeof extractErrorText === "function" ? extractErrorText(error) : normalizeText(error?.message || "");
    ...
    async function createLobbyWithBullModeFallback(lobbyPayload, token, deps = {}) {
      ...
      const extractErrorText = typeof deps.extractErrorText === "function" ? deps.extractErrorText : ((error) => normalizeText(error?.message || ""));
      ...
      try {
        ...
        if (shouldCleanupFailedMatchStartLobby(createdLobbyId, startRequested) && deleteLobbyFn) {
          ...
          try {
            ...
            cleanup.ok = false;
            cleanup.message = normalizeText(extractErrorText(cleanupError) || cleanupError?.message || "Lobby-Cleanup fehlgeschlagen.") || "Lobby-Cleanup fehlgeschlagen.";
          ...
          cleanup.skippedReason = !createdLobbyId ? "no_lobby_created" : startRequested ? "start_already_requested" : "delete_lobby_unavailable";
          emitStep?.({
            step: "cleanup_lobby",
            status: "skipped",
            details: {
    ...
    function isSameX01Settings(left, right) {
      return Boolean(left && right) && left.baseScore === right.baseScore && left.inMode === right.inMode && left.outMode === right.outMode && left.bullMode === right.bullMode && left.maxRounds === right.maxRounds && left.bullOffMode === right.bullOffMode && left.lobbyVisibility === right.lobbyVisibility;
    }
    ...
    function matchesCreatePresetSetup(input, presetId) {
      ...
      const x01Input = input?.x01 && typeof input.x01 === "object" ? input.x01 : input;
    ...
    function normalizeCreateDraft(rawDraft, settings = null) {
      ...
      const requestedPresetId = hasExplicitPreset ? sanitizeX01Preset(rawDraft?.x01Preset, X01_PRESET_CUSTOM) : hasDraftObject ? X01_PRESET_CUSTOM : base.x01Preset;
      ...
      const draft = {
        ...
        randomizeKoRound1: typeof rawDraft?.randomizeKoRound1 === "boolean" ? rawDraft.randomizeKoRound1 : base.randomizeKoRound1,
        enableThirdPlaceMatch: typeof rawDraft?.enableThirdPlaceMatch === "boolean" ? rawDraft.enableThirdPlaceMatch : base.enableThirdPlaceMatch
      };
    ...
    function normalizeTournamentRules(rawRules) {
      ...
      const tieBreakRaw = Object.prototype.hasOwnProperty.call(rules, "tieBreakProfile") ? rules.tieBreakProfile : rules.tieBreakMode;
      return {
    ...
    function getAppliedCreatePresetId(input) {
      ...
      return matchesCreatePresetSetup(input, requestedPreset.id) ? requestedPreset.id : X01_PRESET_CUSTOM;
    }
    ...
    function normalizeKoVirtualMatch(rawMatch, roundFallback, indexFallback) {
      const matchRole = normalizeText(rawMatch?.matchRole || "").toLowerCase() === "third_place" ? "third_place" : "main";
      return {
        ...
        placementRank: Number.isFinite(Number(rawMatch?.placementRank)) ? clampInt(rawMatch?.placementRank, null, 1, 128) : null,
        competitors: {
    ...
    function normalizeKoRoundStructure(rawRound, roundFallback, totalRounds = roundFallback) {
      ...
      return {
        ...
        virtualMatches: virtualMatchesRaw.map((entry, index) => normalizeKoVirtualMatch(entry, roundFallback, index + 1))
      };
    }
    function normalizeKoSeedEntry(rawSeed, indexFallback) {
      ...
      return {
        ...
        slot: Number.isFinite(Number(rawSeed?.slot)) ? clampInt(rawSeed?.slot, null, 1, TECHNICAL_PARTICIPANT_HARD_MAX) : null
      };
    }
    ...
    function normalizeKoPlacement(placementRaw, bracketSize) {
      ...
      const normalized = placementRaw.map((entry) => clampInt(entry, null, 1, bracketSize)).filter((entry) => Number.isInteger(entry) && !used.has(entry) && used.add(entry));
    ...
    function normalizeTournamentKoMeta(rawKo, fallbackDrawMode = KO_DRAW_MODE_SEEDED, fallbackDrawLocked = true) {
      ...
      const seeding = (Array.isArray(ko.seeding) ? ko.seeding : []).map((entry, index) => normalizeKoSeedEntry(entry, index + 1)).filter(Boolean);
      const rounds = (Array.isArray(ko.rounds) ? ko.rounds : []).map((entry, index, array) => normalizeKoRoundStructure(entry, index + 1, array.length || index + 1));
    ...
    function normalizeTournamentResultEntry(rawResult, indexFallback) {
      return {
        ...
        stage: [MATCH_STAGE_KO, MATCH_STAGE_GROUP, MATCH_STAGE_LEAGUE].includes(rawResult?.stage) ? rawResult.stage : MATCH_STAGE_KO,
    ...
    function normalizeTournament(rawTournament, fallbackKoDrawLocked = true) {
      ...
      const participants = participantsRaw.map((entry, index) => {
        ...
        return { id, name };
      }).filter(Boolean).slice(0, TECHNICAL_PARTICIPANT_HARD_MAX);
      ...
      const groups = groupsRaw.map((group, index) => ({
        ...
        participantIds: Array.isArray(group?.participantIds) ? group.participantIds.map((id) => normalizeText(id)).filter(Boolean) : []
      }));
      ...
      return {
        ...
        mode,
        ko: mode === "ko" ? normalizeTournamentKoMeta(rawTournament.ko, KO_DRAW_MODE_SEEDED, fallbackKoDrawLocked) : null,
    ...
    function buildModeParticipantLimitSummary() {
      return Object.entries(MODE_PARTICIPANT_LIMITS).map(([, limits]) => `${limits.label}: ${limits.min}-${limits.max}`).join(", ");
    }
    ...
    function assignPlayerSlot(match, slot, participantId) {
      ...
      const hasStoredResult = match.status === STATUS_COMPLETED || Boolean(match.winnerId || match.source || match.legs?.p1 || match.legs?.p2);
      if (hasStoredResult) {
    ...
    function hasRelevantCompletedTieBreakMatch(tournament) {
      ...
      return matches.some((match) => match?.status === STATUS_COMPLETED && (match.stage === MATCH_STAGE_GROUP || match.stage === MATCH_STAGE_LEAGUE));
    }
    ...
    function createKoVirtualMatch({
      ...
      const normalizedPlacementRank = Number.isFinite(Number(placementRank)) ? clampInt(placementRank, null, 1, 128) : null;
      return {
        id: normalizeText(id || ""),
    ...
    function createMatch({
      ...
      const normalizedWinnerId = normalizedStatus === STATUS_COMPLETED ? normalizeText(winnerId || "") || null : null;
    ...
    function buildBracketStructure(players, seeds, options = {}) {
      ...
      const seeded = Array.isArray(seeds) && seeds.length ? seeds.slice() : generateSeeds(normalizedParticipants, KO_DRAW_MODE_SEEDED);
      ...
      const slotByParticipantId = /* @__PURE__ */ new Map();
      ...
      for (let round = 1; round <= totalRounds; round += 1) {
        ...
        for (let number = 1; number <= matchesInRound; number += 1) {
          ...
          virtualMatches.push(createKoVirtualMatch({
            ...
            structuralBye,
            matchRole: "main",
            advancesWinnerTo: round < totalRounds ? `ko-r${round + 1}-m${Math.ceil(number / 2)}` : null,
            advancesLoserTo: null,
      ...
      if (enableThirdPlaceMatch && totalRounds >= 2) {
        ...
        const semifinalMatches = (Array.isArray(semifinalRoundDef?.virtualMatches) ? semifinalRoundDef.virtualMatches : []).filter((entry) => entry?.matchRole !== "third_place").sort((left, right) => left.number - right.number);
        const isValidThirdPlacePath = semifinalMatches.length === 2 && semifinalMatches.every((entry) => !entry.structuralBye && Boolean(entry?.competitors?.p1) && Boolean(entry?.competitors?.p2));
        if (isValidThirdPlacePath && finalRoundDef) {
          ...
          finalRoundDef.virtualMatches = finalRoundDef.virtualMatches.filter((entry) => entry.id !== thirdPlaceMatch.id).concat(thirdPlaceMatch).sort((left, right) => left.number - right.number);
        }
      }
      return {
    ...
    function buildKoMatchMetaFromVirtualMatch(virtualMatch) {
      return {
        bracket: {
          ...
          placementRank: Number.isFinite(Number(virtualMatch?.placementRank)) ? clampInt(virtualMatch?.placementRank, null, 1, 128) : null
        }
      };
    }
    ...
    function buildKoMatchesFromStructure(bracketStructure) {
      ...
      rounds.forEach((roundDef) => {
        roundDef.virtualMatches.forEach((virtualMatch) => {
          ...
          const meta = isBye ? { ...baseMeta, resultKind: "bye" } : baseMeta;
          matches.push(createMatch({
            id: virtualMatch.id,
    ...
    function buildKoMatchesV2(participantIds, drawMode = KO_DRAW_MODE_SEEDED) {
      const participants = (Array.isArray(participantIds) ? participantIds : []).map((entry) => ({
    ...
    function createTournament(config) {
      ...
      const koDrawMode = config.mode === "ko" && config.randomizeKoRound1 ? KO_DRAW_MODE_OPEN_DRAW : KO_DRAW_MODE_SEEDED;
      const koDrawLocked = config.mode === "ko" ? config.koDrawLocked !== false : false;
      const enableThirdPlaceMatch = config.mode === "ko" ? config.enableThirdPlaceMatch === true : false;
    ...
    function getMatchesByStage(tournament, stage) {
      return tournament.matches.filter((match) => match.stage === stage).sort((left, right) => left.round - right.round || left.number - right.number);
    }
    ...
    const TOURNAMENT_DURATION_RESULT_ENTRY_MINUTES = 0.8;
    const TOURNAMENT_DURATION_LOW_FACTOR = 0.9;
    ...
    const TOURNAMENT_TIME_PROFILE_META = Object.freeze({
      ...
      [TOURNAMENT_TIME_PROFILE_NORMAL]: Object.freeze({
        ...
        legPaceMultiplier: 1,
        matchTransitionMinutes: 0.8,
        phaseTransitionMultiplier: 1,
        highPaddingExtra: 0
      }),
    ...
    function getExpectedLegsForBestOf(bestOfLegs) {
      ...
      for (let totalLegs = legsToWin; totalLegs < legsToWin * 2; totalLegs += 1) {
        const probability = 2 * getTournamentDurationCombination(totalLegs - 1, legsToWin - 1) * Math.pow(0.5, totalLegs);
    ...
    function normalizeTournamentDurationTaskList(rawTasks) {
      ...
      source.forEach((task, index) => {
        ...
        seen.add(taskId);
        const participants = (Array.isArray(task?.participants) ? task.participants : []).map((entry) => normalizeText(entry || "")).filter(Boolean);
        const dependsOn = (Array.isArray(task?.dependsOn) ? task.dependsOn : []).map((entry) => normalizeText(entry || "")).filter(Boolean);
        normalized.push({
          id: taskId,
          participants,
          dependsOn
    ...
    function estimateTournamentDurationSchedule(rawTasks, boardCount) {
      ...
      const remainingDependenciesById = /* @__PURE__ */ new Map();
      tasks.forEach((task) => {
      ...
      const criticalDepthById = /* @__PURE__ */ new Map();
      tasks.forEach((task) => {
        getTournamentDurationTaskDepth(task.id, dependentsById, criticalDepthById, /* @__PURE__ */ new Set());
      });
      ...
      while (unscheduled.size > 0) {
        const ready = tasks.filter((task) => unscheduled.has(task.id) && (remainingDependenciesById.get(task.id) || 0) <= 0).sort((left, right) => {
        ...
        const usedParticipants = /* @__PURE__ */ new Set();
        const selected = [];
      ...
      const boardUtilization = waves > 0 ? scheduledMatches / (waves * safeBoardCount) : 0;
      return {
        waves,
        peakParallelMatches,
    ...
    function buildKoTournamentDurationTasks(participantCount, enableThirdPlaceMatch = false) {
      ...
      const playableMatchIds = /* @__PURE__ */ new Set();
    ...
    function estimateTournamentDurationProgressFromTournament(tournament, settings = null) {
      ...
      const remainingLikelyMinutes = remainingWaves * totalEstimate.matchMinutes + totalEstimate.phaseOverheadMinutes * (remainingTasks.length / tasks.length);
      const remainingLowMinutes = remainingLikelyMinutes * TOURNAMENT_DURATION_LOW_FACTOR;
      const likelyHighFactor = totalEstimate.likelyMinutes > 0 ? totalEstimate.highMinutes / totalEstimate.likelyMinutes : 1 + TOURNAMENT_DURATION_HIGH_BASE_PADDING;
    ...
    function estimateTournamentDuration(rawInput, settings = null) {
      ...
      const participants = (Array.isArray(rawInput?.participants) ? rawInput.participants : []).filter((entry) => normalizeText(entry?.id || entry?.name || entry || ""));
      ...
      const bullModeFactor = x01Settings.bullOffMode === "Off" ? 1 : TOURNAMENT_DURATION_BULL_FACTORS[x01Settings.bullMode] || 1;
      const legMinutes = TOURNAMENT_DURATION_BASE_LEG_MINUTES * profile.legPaceMultiplier * (TOURNAMENT_DURATION_SCORE_FACTORS[x01Settings.baseScore] || 1) * (TOURNAMENT_DURATION_IN_FACTORS[x01Settings.inMode] || 1) * (TOURNAMENT_DURATION_OUT_FACTORS[x01Settings.outMode] || 1) * bullModeFactor;
      const bullOffOverheadMinutes = TOURNAMENT_DURATION_BULL_OFF_OVERHEAD[x01Settings.bullOffMode] || 0;
      const matchOverheadMinutes = TOURNAMENT_DURATION_RESULT_ENTRY_MINUTES + profile.matchTransitionMinutes + bullOffOverheadMinutes;
      ...
      const scheduleWaves = schedule.waves > 0 ? schedule.waves : matchCount > 0 ? Math.ceil(matchCount / boardCount) : 0;
      const phaseOverheadMinutes = getTournamentDurationPhaseOverheadMinutes(mode, participantCount) * profile.phaseTransitionMultiplier;
      const likelyMinutes = scheduleWaves * matchMinutes + phaseOverheadMinutes;
      const highPadding = TOURNAMENT_DURATION_HIGH_BASE_PADDING + (TOURNAMENT_DURATION_MAX_ROUNDS_HIGH_PADDING[x01Settings.maxRounds] || 0) + getTournamentDurationDifficultyPadding(x01Settings) + profile.highPaddingExtra;
      estimate.ready = true;
      ...
      estimate.averageParallelMatches = scheduleWaves > 0 ? matchCount / scheduleWaves : 0;
      estimate.peakParallelMatches = schedule.peakParallelMatches > 0 ? schedule.peakParallelMatches : matchCount > 0 ? Math.min(boardCount, matchCount) : 0;
      estimate.boardUtilization = scheduleWaves > 0 ? matchCount / (scheduleWaves * boardCount) : 0;
    ...
    function standingsForMatches(tournament, matches, participantIds = null) {
      const allowedIds = Array.isArray(participantIds) ? new Set(participantIds.map((id) => normalizeText(id)).filter(Boolean)) : null;
      const rows = (tournament?.participants || []).filter((participant) => !allowedIds || allowedIds.has(participant.id)).map((participant) => ({
        id: participant.id,
      ...
      if (tieBreakProfile === TIE_BREAK_PROFILE_PROMOTER_H2H_MINITABLE) {
        const pointsBuckets = /* @__PURE__ */ new Map();
        rows.forEach((row) => {
        ...
        pointsBuckets.forEach((bucketRows) => {
          ...
          const bucketMatches = completedMatches.filter((match) => match.player1Id && match.player2Id && bucketIds.has(match.player1Id) && bucketIds.has(match.player2Id));
          if (bucketRows.length === 2) {
      ...
      if (tieBreakProfile === TIE_BREAK_PROFILE_PROMOTER_H2H_MINITABLE) {
        const unresolvedBuckets = /* @__PURE__ */ new Map();
        rows.forEach((row) => {
          const key = [
            row.points,
    ...
    function resolveGroupsToKoAssignments(tournament) {
      ...
      semifinals.forEach((match) => {
        ...
        const p1 = group1 && group1.complete && group1.groupResolution?.status === "resolved" ? group1.rows[from1.rank - 1]?.id || null : null;
        const p2 = group2 && group2.complete && group2.groupResolution?.status === "resolved" ? group2.rows[from2.rank - 1]?.id || null : null;
    ...
    function buildKoMetaSnapshot(drawMode, drawLocked, structure) {
      ...
      const placement = (Array.isArray(structure?.placement) ? structure.placement : []).map((slot) => clampInt(slot, null, 1, bracketSize)).filter((slot) => Number.isInteger(slot));
      const normalizedPlacement = placement.length === bracketSize ? placement : buildSeedPlacement(bracketSize);
      const rounds = (Array.isArray(structure?.rounds) ? structure.rounds : []).map((roundDef) => ({
        ...
        virtualMatches: (Array.isArray(roundDef?.virtualMatches) ? roundDef.virtualMatches : []).map((virtualMatch) => ({
          ...
          matchRole: normalizeText(virtualMatch?.matchRole || "").toLowerCase() === "third_place" ? "third_place" : "main",
          advancesWinnerTo: normalizeText(virtualMatch?.advancesWinnerTo || "") || null,
          advancesLoserTo: normalizeText(virtualMatch?.advancesLoserTo || "") || null,
          placementRank: Number.isFinite(Number(virtualMatch?.placementRank)) ? clampInt(virtualMatch?.placementRank, null, 1, 128) : null,
          competitors: {
    ...
    function synchronizeKoBracketState(tournament) {
      ...
      const participants = (Array.isArray(tournament.participants) ? tournament.participants : []).map((participant) => ({
      ...
      const winnerByVirtualMatchId = /* @__PURE__ */ new Map();
      const loserByVirtualMatchId = /* @__PURE__ */ new Map();
      const nextKoMatches = [];
      structure.rounds.forEach((roundDef) => {
        roundDef.virtualMatches.forEach((virtualMatch) => {
    ...
    function isCompletedMatchResultValid(tournament, match) {
      ...
      if (isByeMatchResult(match)) {
        ...
        const p1Legs = clampInt(match.legs?.p1, 0, 0, 99);
        const p2Legs = clampInt(match.legs?.p2, 0, 0, 99);
        return normalizeText(match.winnerId || "") === normalizeText(expectedWinnerId || "") && p1Legs === 0 && p2Legs === 0;
      }
      if (!match.player1Id || !match.player2Id) {
        return false;
      }
    ...
    function buildTournamentResults(tournament) {
      const stageOrder = /* @__PURE__ */ new Map([
        [MATCH_STAGE_GROUP, 1],
        [MATCH_STAGE_LEAGUE, 2],
        [MATCH_STAGE_KO, 3]
      ]);
      return (Array.isArray(tournament?.matches) ? tournament.matches : []).filter((match) => match?.status === STATUS_COMPLETED && isCompletedMatchResultValid(tournament, match)).map((match) => ({
        matchId: match.id,
        stage: match.stage,
        round: match.round,
        number: match.number,
        ...
        updatedAt: normalizeText(match.updatedAt || nowIso())
      })).sort((left, right) => (stageOrder.get(left.stage) || 99) - (stageOrder.get(right.stage) || 99) || left.round - right.round || left.number - right.number);
    }
    ...
    function getOpenMatchByPlayers(tournament, player1Id, player2Id) {
      const key = /* @__PURE__ */ new Set([player1Id, player2Id]);
      const candidates = tournament.matches.filter((match) => {
        ...
        const set = /* @__PURE__ */ new Set([match.player1Id, match.player2Id]);
    ...
    function getKoBlockingSourceMatch(tournament, match) {
      ...
      const sourceMatchIds = /* @__PURE__ */ new Set();
      ...
      if (sourceMatchIds.size) {
        const sourceMatches2 = getMatchesByStage(tournament, MATCH_STAGE_KO).filter((item) => sourceMatchIds.has(normalizeText(item?.id || "")));
        return sourceMatches2.find((item) => item.status !== STATUS_COMPLETED) || null;
      }
      const previousRound = match.round - 1;
      const sourceNumberA = (match.number - 1) * 2 + 1;
      const sourceNumberB = sourceNumberA + 1;
      const sourceMatches = getMatchesByStage(tournament, MATCH_STAGE_KO).filter((item) => item.round === previousRound && (item.number === sourceNumberA || item.number === sourceNumberB)).sort((left, right) => left.number - right.number);
    ...
    function buildBracketPayload(tournament) {
      ...
      const bracketSize = tournament.mode === "groups_ko" ? 4 : nextPowerOfTwo(clampInt(tournament?.ko?.bracketSize, tournament.participants.length, 2, TECHNICAL_PARTICIPANT_HARD_MAX));
      ...
      const matches = koMatches.map((match) => {
        ...
        const completed = isCompletedMatchResultValid(tournament, match) && Boolean(winnerId && (winnerId === player1Id || winnerId === player2Id));
        const occupied = Boolean(player1Id || player2Id);
        const status = completed ? 4 : occupied ? 2 : 1;
        const opponent1 = player1Id ? {
          id: player1Id,
          score: completed ? clampInt(match.legs?.p1, 0, 0, 99) : void 0,
          result: completed && winnerId ? winnerId === player1Id ? "win" : isBye ? void 0 : "loss" : void 0
        } : null;
        const opponent2 = player2Id ? {
          id: player2Id,
          score: completed ? clampInt(match.legs?.p2, 0, 0, 99) : void 0,
          result: completed && winnerId ? winnerId === player2Id ? "win" : isBye ? void 0 : "loss" : void 0
        } : null;
        return {
          id: match.id,
          stage_id: 1,
...
<body>
  ...
  <\/script>
    ...
    function applyBracketFrameHeight(frame, bracketState, height) {
      ...
      const nextHeight = clampInt(height, 0, 420, 12e3);
    ...
    const DRAW_UNLOCK_OVERRIDE_WINDOW_MS = 3e4;
    ...
    function findSuggestedNextMatch(tournament) {
      const source = Array.isArray(tournament?.matches) ? tournament.matches.slice() : [];
      const candidates = source.filter((match) => {
    ...
    function syncLoaderMenuUpdateIndicator() {
      ...
      const title = hasUpdate && remoteVersion ? `xLokales Turnier - Update verfügbar (${APP_VERSION} -> ${remoteVersion})` : "xLokales Turnier";
    ...
    function runSelfTests() {
      ...
      try {
        ...
        const toSignature = (matches) => matches.map((match) => `${match.id}:${match.player1Id || "-"}:${match.player2Id || "-"}:${isByeMatchResult(match) ? "bye" : "match"}`).join("|");
      ...
      try {
        ...
        record(
          ...
          presetChecks.every((entry) => entry.ok) && europeanTourPreset?.apply?.bestOfLegs === 11 && europeanTourPreset?.apply?.startScore === 501,
      ...
      }
      {
        ...
        try {
          ...
          record(
            ...
            europeanTourDraft.x01Preset === X01_PRESET_PDC_EUROPEAN_TOUR_OFFICIAL && europeanTourDraft.mode === "ko" && europeanTourDraft.bestOfLegs === 11 && basicDraft.x01Preset === X01_PRESET_PDC_501_DOUBLE_OUT_BASIC && basicDraft.bestOfLegs === 5 && basicDraft.startScore === 501 && basicDraft.x01OutMode === "Double",
      ...
      try {
        ...
        record(
          ...
          payload?.variant === X01_VARIANT && payload?.isPrivate === true && payload?.bullOffMode === "Official" && payload?.legs === 4 && payload?.settings?.baseScore === 701 && payload?.settings?.inMode === "Double" && payload?.settings?.outMode === "Master" && payload?.settings?.maxRounds === 20 && payload?.settings?.bullMode === "50/50",
      ...
      try {
        ...
        record(
          ...
          payload?.bullOffMode === "Off" && hasBullMode,
      ...
      try {
        ...
        record(
          ...
          report?.sessionCount === 1 && Array.isArray(report?.sessions) && report.sessions[0]?.matchId === "dbg-m-1" && report.sessions[0]?.lobbyId === "dbg-lobby-1",
      ...
      try {
        ...
        record(
          ...
          blocked?.ok === false && blocked?.reasonCode === "draw_unlock_requires_override" && Boolean(confirmed?.ok && confirmed?.changed) && tournament?.ko?.drawLocked === false,
      ...
      try {
        ...
        record(
          ...
          result?.ok === false && result?.reasonCode === "tie_break_locked" && tournament.rules.tieBreakProfile === TIE_BREAK_PROFILE_PROMOTER_H2H_MINITABLE,
      ...
      try {
        ...
        record(
          ...
          blocked?.ok === false && blocked?.reasonCode === "tie_break_locked" && beforeSemiState === afterSemiState,
      ...
      try {
        record(
          ...
          isAutoDetectMatchRoute("/matches/abc123") && isAutoDetectMatchRoute("/lobbies/abc123") && !isAutoDetectMatchRoute("/history/matches/abc123") && !isAutoDetectMatchRoute("/settings"),
      ...
      }
      {
        ...
        `;
          ...
          record(
            ...
            outcome?.reasonCode === "requires_confirmation" && Boolean(confirmationSignature) && confirmed?.reasonCode === "completed" && updated?.status === STATUS_COMPLETED && updated?.winnerId === "P1" && updated?.legs?.p1 === 2 && updated?.legs?.p2 === 0,
      ...
      }
      {
        ...
        `;
          ...
          if (pending) {
            pending.expiresAt = Date.now() - 1e3;
          }
          ...
          record(
            ...
            needsConfirm?.reasonCode === "requires_confirmation" && invalidConfirm?.reasonCode === "confirmation_invalid" && expiredConfirm?.reasonCode === "confirmation_expired",
      ...
      }
      {
        ...
        `;
          ...
          record(
            ...
            Boolean(outcome?.ok) && matchA?.status === STATUS_PENDING && matchB?.status === STATUS_COMPLETED && matchB?.winnerId === "P1",
      ...
      `;
        ...
        record(
          ...
          ambiguousHost?.reasonCode === "history_host_ambiguous" && missingHost?.reasonCode === "history_host_not_found",
      ...
      try {
        const comparisonsOk = compareVersions("0.3.4", "0.3.3") > 0 && compareVersions("0.3.3", "0.3.3") === 0 && compareVersions("0.3.3-beta", "0.3.3") < 0;
...
`);
      ...
      try {
        ...
        const requestUrl = new URL(buildCacheBustedUrl(USERSCRIPT_UPDATE_URL, 1770301234567));
        record(
          "Update-Check: gecachter Status und Cache-Bust-URL werden konsistent abgeleitet",
          status.capable === true && resolved.available === true && resolved.remoteVersion === "9.9.9" && requestUrl?.searchParams?.get(UPDATE_CACHE_BUST_PARAM) === "1770301234567" && status.validators?.[USERSCRIPT_UPDATE_URL]?.etag === '"ata-update"',
      ...
      try {
        ...
        record(
          ...
          bearer === "test.token.value" && plain === "test.token.value" && invalidBasic === "" && invalidOtherScheme === "",
      ...
      try {
        ...
        try {
          ...
          record(
            ...
            ignored === "" && captured === "captured.token" && snapshot.hasCachedToken === true && state.apiAutomation.authTokenSource === "selftest:xhr",
      ...
      try {
        ...
        try {
          ...
          record(
            ...
            snapshot.hasCachedToken === true && state.apiAutomation.authTokenSource === "selftest:bridge",
      ...
      try {
        ...
        try {
          ...
          cacheResolvedAuthToken("header.payload.signature", "selftest", Date.now() + 12e4);
          const snapshotWithCache = getAuthStateSnapshot();
          record(
            "API Auth: Snapshot erkennt Refresh-Token- und Cache-Kontext",
            snapshotWithRefresh.hasRefreshToken === true && snapshotWithRefresh.hasAnyAuthContext === true && snapshotWithCache.hasCachedToken === true && snapshotWithCache.cachedTokenUsable === true,
      ...
      try {
        ...
        record(
          ...
          migrated.schemaVersion === 4 && migrated.tournament?.rules?.tieBreakProfile === TIE_BREAK_PROFILE_PROMOTER_H2H_MINITABLE && migrated.settings?.tournamentTimeProfile === TOURNAMENT_TIME_PROFILE_NORMAL && migrated.settings?.featureFlags?.koDrawLockDefault === true,
    ...
    function cacheResolvedAuthToken(token, source = "", fallbackExpiryMs = 0) {
      ...
      state.apiAutomation.authTokenExpiresAt = normalizedToken ? getTokenExpiryMs(normalizedToken, fallbackExpiryMs) : 0;
    ...
    function isCachedAuthTokenUsable() {
      ...
      return Date.now() < expiresAt - 30 * 1e3;
    }
...
`;
      ...
      if (!state.apiAutomation.authHeaderCaptureInstalled) {
        ...
        const bridgeEventListener = (event) => {
          try {
            ...
            const source = normalizeText(detail.source || AUTH_HEADER_CAPTURE_DEFAULT_SOURCE) || AUTH_HEADER_CAPTURE_DEFAULT_SOURCE;
    ...
    function getAuthStateSnapshot() {
      ...
      return {
        ...
        source: hasCookieToken ? "cookie" : hasCachedToken ? "cache" : hasRefreshToken ? "refresh-token" : "none"
      };
    }
    ...
    async function resolveAuthToken(options = {}) {
      ...
      state.apiAutomation.authRefreshPromise = (async () => {
        try {
          ...
          const expiresInMs = Math.max(0, Number(refreshed?.expires_in || 0)) * 1e3;
    ...
    function collectRuntimeStatus() {
      ...
      return {
        ...
        boardLabel: hasBoard ? `Board aktiv (${boardPreview})` : hasBoardValue ? `Board-ID ungültig (${boardPreview})` : "Kein aktives Board",
    ...
    function renderRuntimeStatusBar() {
      ...
      const hint = status.autoEnabled && (!status.hasToken || !status.hasBoard) ? renderDocLinkableMessage("Hinweis: Für API-Halbautomatik werden Auth-Token und aktives Board benötigt.", {
        tagName: "span",
    ...
    async function requestJsonViaGm(method, url, payload, headers) {
      return new Promise((resolve, reject) => {
        GM_xmlhttpRequest({
          ...
          headers,
          data: payload ? JSON.stringify(payload) : void 0,
          onload: (response) => {
    ...
    function resolveParticipantIdFromApiRef(tournament, participantRef) {
      ...
      const direct = (tournament?.participants || []).find((participant) => normalizeText(participant?.id || "") === normalizedRef);
      if (direct?.id) {
        return direct.id;
      }
      const lookup = normalizeLookup(normalizedRef);
      const byName = (tournament?.participants || []).find((participant) => normalizeLookup(participant?.name || "") === lookup);
      return byName?.id || "";
    }
    function getOpenMatchesByPlayersPair(tournament, player1Id, player2Id) {
      const key = /* @__PURE__ */ new Set([normalizeText(player1Id || ""), normalizeText(player2Id || "")]);
      ...
      return (tournament?.matches || []).filter((match) => {
        ...
        const set = /* @__PURE__ */ new Set([normalizeText(match.player1Id), normalizeText(match.player2Id)]);
        return key.size === set.size && [...key].every((id) => set.has(id));
      });
    }
    ...
    function resolveWinnerIdCandidatesFromApiStats(tournament, match, data, winnerIndex) {
      ...
      matchStats.forEach((entry) => {
        const hasWinnerFlag = entry?.winner === true || entry?.isWinner === true || entry?.won === true || entry?.result === "winner" || entry?.result === "win";
        if (hasWinnerFlag) {
    ...
    function getApiNameToLegsLookup(data) {
      ...
      for (let index = 0; index < count; index += 1) {
        ...
        const statName = normalizeLookup(
          statEntry?.name || statEntry?.playerName || statEntry?.player?.name || statEntry?.player?.username || ""
        );
    ...
    function getApiMatchStartUi(tournament, match, activeStartedMatch) {
      ...
      if (!isValidBoardId(boardId)) {
        return {
          label: "Match starten",
          disabled: true,
          title: boardId ? `Board-ID ungültig (${boardId}). Bitte Board in einer manuellen Lobby wählen.` : "Kein Board aktiv. Bitte einmal manuell eine Lobby öffnen und Board wählen."
        };
      }
    ...
    async function handleStartMatch(matchId) {
      ...
      const debugSession = state.store.settings.debug ? createMatchStartDebugSession({
    ...
    function scanForAutoResult() {
      ...
      if (!targetMatch) {
        const openWithWinner = tournament.matches.filter((match) => match.status === STATUS_PENDING && match.player1Id && match.player2Id && (match.player1Id === winnerId || match.player2Id === winnerId));
    ...
    const HISTORY_IMPORT_CONFIRMATION_TTL_MS = 45e3;
    ...
    function findHistoryImportHost(lobbyId) {
      ...
      const routeLinks = Array.from(document.querySelectorAll(`a[href^="/history/matches/${targetLobbyId}"]`)).filter((link) => link instanceof HTMLAnchorElement);
      ...
      routeLinks.forEach((link) => {
        const card2 = link.closest(".chakra-card, [class*='chakra-card'], article, section, [class*='card']");
        if (!(card2 instanceof HTMLElement)) {
          return;
        }
        if (seenCards.has(card2)) {
          return;
        }
        seenCards.add(card2);
        cards.push(card2);
    ...
    function parseHistoryStatsTable(table) {
      ...
      rows.forEach((row) => {
        ...
        const isLegsRow = label.includes("gewonnene legs") || label.includes("legs won") || label === "legs";
        if (!isLegsRow) {
          return;
        }
    ...
    function scoreParticipantNameMatch(participantName, tableName) {
      ...
      if (participantLookup.startsWith(`${tableLookup} `) || participantLookup.endsWith(` ${tableLookup}`) || tableLookup.startsWith(`${participantLookup} `) || tableLookup.endsWith(` ${participantLookup}`)) {
        return 280;
      }
    ...
    function participantIdsByName(tournament, name) {
      ...
      })).filter((entry) => entry.id && entry.score > 0).sort((left, right) => right.score - left.score);
      if (!scored.length) {
        return [];
      }
    ...
    function getOpenMatchCandidatesByParticipantIds(tournament, idA, idB) {
      ...
      const key = /* @__PURE__ */ new Set([left, right]);
      return (tournament?.matches || []).filter((match) => {
        if (!match || match.status !== STATUS_PENDING || !match.player1Id || !match.player2Id) {
          return false;
        }
    ...
    function resolveTableToMatchOrder(tournament, match, parsed) {
      ...
      const directScore = scoreParticipantNameMatch(matchP1Name, parsed?.p1Name) + scoreParticipantNameMatch(matchP2Name, parsed?.p2Name);
      const swappedScore = scoreParticipantNameMatch(matchP1Name, parsed?.p2Name) + scoreParticipantNameMatch(matchP2Name, parsed?.p1Name);
    ...
    function importHistoryStatsTableResult(lobbyId, hostInfo, options = {}) {
      ...
      const legsRaw = tableMapsDirect ? { p1: parsed.p1Legs, p2: parsed.p2Legs } : { p1: parsed.p2Legs, p2: parsed.p1Legs };
      let winnerId = "";
      if (parsed.winnerIndex === 0) {
      ...
      if (normalizedLegs.adjusted) {
        if (!requestedSignature) {
          ...
          setPendingHistoryConfirmation(targetLobbyId, pending2);
          return {
            ok: true,
            completed: false,
            reasonCode: "requires_confirmation",
            requiresConfirmation: true,
            message: `Leg-Abweichung erkannt: Statistik ${formatHistoryLegsText(pending2.legsRaw)} -> Turnier First to ${normalizedLegs.legsToWin} erwartet ${formatHistoryLegsText(pending2.normalizedLegs)}. Bitte explizit bestätigen.`,
            confirm: {
              signature: pending2.signature,
              expiresAt: pending2.expiresAt,
              ttlMs: HISTORY_IMPORT_CONFIRMATION_TTL_MS,
              matchId: pending2.matchId,
              winnerId: pending2.winnerId,
              legsRaw: pending2.legsRaw,
              normalizedLegs: pending2.normalizedLegs,
              legsToWin: pending2.legsToWin
            }
          };
        }
      ...
      const successMessage = normalizedLegs.adjusted ? `Ergebnis übernommen. Legs wurden nach bestätigter Abweichung auf First to ${normalizedLegs.legsToWin} gesetzt.` : "Ergebnis wurde aus der Match-Statistik übernommen.";
      return {
        ok: true,
        completed: true,
        reasonCode: "completed",
    ...
    function renderHistoryImportButton() {
      ...
      const parsedScoreText = parsedStats ? `${parsedStats.p1Name} ${parsedStats.p1Legs}:${parsedStats.p2Legs} ${parsedStats.p2Name}` : "";
      ...
      const primaryLabel = isAlreadyCompleted ? "Turnierassistent öffnen" : isSyncing ? "Übernehme..." : pendingConfirmation ? "Statistik erneut prüfen" : "Ergebnis aus Statistik übernehmen & Turnier öffnen";
      ...
      const outcomeColor = outcomeType === "success" ? "#d8ffe7" : outcomeType === "error" ? "#ffd9dc" : "#dbe8ff";
      const outcomeBg = outcomeType === "success" ? "rgba(73, 205, 138, 0.16)" : outcomeType === "error" ? "rgba(214, 74, 105, 0.18)" : "rgba(119, 167, 255, 0.14)";
      const outcomeBorder = outcomeType === "success" ? "rgba(95, 220, 154, 0.5)" : outcomeType === "error" ? "rgba(245, 123, 143, 0.52)" : "rgba(142, 188, 255, 0.48)";
      ...
      const outcomeMessageHtml = outcomeMessage ? renderDocLinkableMessage(outcomeMessage, {
        tagName: "div",
    ...
    async function handleLobbySyncAndOpen(lobbyId, trigger = "manual", options = {}) {
      ...
      try {
        ...
        if (syncOutcome.reasonCode === "completed" || syncOutcome.completed) {
          ...
          const isError = reasonCode === "ambiguous" || reasonCode === "confirmation_invalid" || reasonCode === "confirmation_expired";
    ...
    function createResolvedUpdateStatus({
      ...
      const comparison = normalizedRemoteVersion ? compareVersions(normalizedRemoteVersion, baseStatus.installedVersion) : 0;
      return {
        ...baseStatus,
        status: normalizedRemoteVersion ? comparison > 0 ? "available" : "current" : error ? "error" : "idle",
    ...
    function renderInfoLinks(links) {
      ...
      const linksHtml = links.map((entry) => {
        ...
        const className = isRuleLink ? "ata-help-link ata-help-link-rule" : "ata-help-link ata-help-link-tech";
    ...
    `).join("");
      const noticeHtml = state.notice.message ? renderDocLinkableMessage(state.notice.message, {
        tagName: "div",
    ...
    function renderTournamentTab() {
      ...
      const tournamentTimeProfileOptions = TOURNAMENT_TIME_PROFILES.map((profileId) => {
        ...
        const label = profileId === TOURNAMENT_TIME_PROFILE_NORMAL ? `${profileMeta.label} (empfohlen)` : profileMeta.label;
      ...
      if (!tournament) {
        ...
        const startScoreOptions = X01_START_SCORE_OPTIONS.map((score) => `<option value="${score}" ${draft.startScore === score ? "selected" : ""}>${score}</option>`).join("");
        const durationEstimate2 = estimateTournamentDurationFromDraft(draft, state.store.settings);
        const activePresetId2 = getAppliedCreatePresetId(draft);
        const presetStatusLabel = `Preset aktiv: ${getCreatePresetLabel(activePresetId2)}`;
        const presetOptions = [
          ...getCreatePresetCatalog().map((preset) => `<option value="${preset.id}" ${draft.x01Preset === preset.id ? "selected" : ""}>${escapeHtml(preset.label)}</option>`),
        ...
        const bullModeHiddenInput = bullModeDisabled ? `<input type="hidden" id="ata-x01-bullmode-hidden" name="x01BullMode" value="${escapeHtml(draft.x01BullMode)}">` : "";
            ...
            <div class="ata-create-layout">
              ...
              <aside class="ata-create-side">
                ...
                <div id="ata-create-duration-estimate">
                  ${renderTournamentDurationEstimate(durationEstimate2, { visible: durationEstimateVisible })}
                </div>
      ...
      const modeLabel = tournament.mode === "ko" ? "KO (Straight Knockout)" : tournament.mode === "league" ? "Liga (Round Robin)" : "Gruppenphase + KO (Round Robin + Straight Knockout)";
      const participantsHtml = tournament.participants.map((participant) => `<span class="ata-player-chip">${escapeHtml(participant.name)}</span>`).join("");
      ...
      const x01BullModeLabel = x01Settings.bullOffMode === "Off" ? "Bull-Modus deaktiviert" : `Bull-Modus ${x01Settings.bullMode}`;
      const legsToWin = getLegsToWin(tournament.bestOfLegs);
      const drawMode = normalizeKoDrawMode(tournament?.ko?.drawMode, KO_DRAW_MODE_SEEDED);
      ...
      const thirdPlaceLabel = tournament?.ko?.enableThirdPlaceMatch === true ? "Spiel um Platz 3: aktiv" : "Spiel um Platz 3: aus";
      const primaryTags = [
    ...
    function renderMatchesTab() {
      ...
      const cards = matches.map((match) => {
        ...
        const isKoFinal = match.stage === MATCH_STAGE_KO && !isThirdPlaceMatch && koFinalRound > 0 && Number(match.round) === koFinalRound;
        const koRoundLabel = match.stage === MATCH_STAGE_KO && !isThirdPlaceMatch ? getKoRoundLabel(match.round, koFinalRound || match.round) : "";
        const stageLabel = match.stage === MATCH_STAGE_GROUP ? `Gruppe ${match.groupId || "?"}` : match.stage === MATCH_STAGE_LEAGUE ? "Liga (Round Robin)" : isThirdPlaceMatch ? "KO (Spiel um Platz 3)" : "KO (Straight Knockout)";
        ...
        if (match.status === STATUS_PENDING) {
          if (!editable && playability.reason) {
            statusLine = auto.status === "idle" ? playability.reason : `${playability.reason} - ${autoStatus}`;
        ...
        const matchCellText = isThirdPlaceMatch ? "Spiel um Platz 3" : match.stage === MATCH_STAGE_KO ? getKoRoundMatchLabel(match.round, koFinalRound || match.round, match.number) : `Runde ${match.round} / Spiel ${match.number}`;
        const matchCellHelpText = isThirdPlaceMatch ? "Spiel um Platz 3 = separates Bronze-Match, getrennt vom Champion-Pfad." : match.stage === MATCH_STAGE_KO ? `KO-Phase = ${koRoundLabel} bzw. bei großen Feldern Letzte N, Spiel = Paarung innerhalb dieser Phase.` : "Runde = Turnierrunde, Spiel = Paarung innerhalb dieser Runde.";
        ...
        const contextPillClass = isByeCompletion ? "ata-match-context-pill ata-match-context-bye" : isCompleted ? "ata-match-context-pill ata-match-context-completed" : "ata-match-context-pill ata-match-context-open";
        const contextText = `${stageLabel}, ${matchCellText}, ${statusBadgeText}`;
        const summaryText = isCompleted ? isByeCompletion ? `Weiter (Bye): ${winner}` : isKoFinal ? `Champion: ${winner} (${match.legs.p1}:${match.legs.p2})` : isThirdPlaceMatch ? `Platz 3: ${winner} (${match.legs.p1}:${match.legs.p2})` : `Sieger: ${winner} (${match.legs.p1}:${match.legs.p2})` : "";
        ...
        ` : "";
        const summaryHtml = summaryText ? `<span class="${escapeHtml(advanceClasses)}">${escapeHtml(summaryText)}</span>` : "";
        const nextPillHtml = isSuggestedNext ? `<span class="ata-match-next-pill" title="Empfohlene nächste Paarung (PDC: Next Match)">Nächstes Match</span>` : "";
        const finalPillHtml = isKoFinal ? `<span class="ata-match-final-pill" title="Finale">🏆 Finale</span>` : "";
        const statusLineHtml = statusLine ? renderDocLinkableMessage(statusLine, {
          tagName: "div",
      ...
      const nextHintHtml = suggestedNextMatchId ? `<p class="ata-small ata-next-hint">Hinweis: Die Markierung "Nächstes Match" zeigt die empfohlene nächste Paarung (PDC: Next Match) ${nextMatchHelpLinks}.</p>` : "";
    ...
    function renderStaticBracketFallbackMatches(tournament, matches) {
      return matches.sort((a, b) => a.number - b.number).map((match) => {
        ...
        const statusBadgeClass = isBye ? "ata-match-status ata-match-status-bye" : isCompleted ? "ata-match-status ata-match-status-completed" : "ata-match-status ata-match-status-open";
        const statusBadgeText = isBye ? "Freilos (Bye)" : isCompleted ? "Abgeschlossen" : "Offen";
        const statusText = !isCompleted ? "Noch nicht abgeschlossen." : isBye ? `Freilos (Bye): ${escapeHtml(participantNameById(tournament, match.winnerId))}` : `Gewinner: ${escapeHtml(participantNameById(tournament, match.winnerId))} (${match.legs.p1}:${match.legs.p2})`;
        return `
    ...
    function renderStaticBracketFallback(tournament) {
      ...
      const mainRoundHtml = [...rounds.entries()].sort((left, right) => left[0] - right[0]).map(([roundNumber, matches]) => `
    ...
    function renderUpdatePanel() {
      ...
      if (panelState === "available") {
        ...
        copyText = loaderActive ? `Installiert: v${installedVersion}. Auf GitHub liegt bereits v${remoteVersion}. Da der Loader aktiv ist, reicht ein Reload von play.autodarts.io.` : `Installiert: v${installedVersion}. Auf GitHub liegt bereits v${remoteVersion}.`;
      } else if (panelState === "current") {
        titleText = "Version ist aktuell";
        copyText = remoteVersion ? `Installiert ist bereits die aktuelle GitHub-Version v${remoteVersion}.` : `Installierte Version: v${installedVersion}.`;
      ...
      <section class="ata-card tournamentCard ata-update-panel ata-update-panel-${escapeHtml(panelState || "checking")}">
        ...
        <div class="ata-update-head">
          ...
          <div class="ata-actions ata-update-actions">
            ...
            ${panelState === "available" ? loaderActive ? `<button type="button" class="ata-btn ata-btn-sm ata-btn-primary" data-action="reload-update">Neu laden</button>` : `<button type="button" class="ata-btn ata-btn-sm ata-btn-primary" data-action="install-update">Update installieren</button>` : ""}
          </div>
        </div>
    ...
    function renderSettingsTab() {
      ...
      const activeKoDrawLocked = state.store?.tournament?.mode === "ko" ? state.store?.tournament?.ko?.drawLocked !== false ? "checked" : "" : "";
      ...
      <section class="ata-card tournamentCard">
        ...
        ${hasMatchStartDebugSessions ? `<pre class="ata-debug-log">${escapeHtml(debugReportText)}</pre>` : `<p class="ata-small">Noch keine Matchstart-Debugdaten vorhanden. Debug-Mode aktivieren, Match testen und das Protokoll danach hier kopieren.</p>`}
        <div class="ata-toggle">
          <div>
      ...
      const updatePromise = resolveLatestUpdateStatus({
        ...
        if (announce) {
          if (nextStatus.status === "available") {
            const actionText = isLoaderRuntimeActive() ? `Neue Version gefunden: ${APP_VERSION} -> ${nextStatus.remoteVersion}. Ein Reload reicht, da der Loader aktiv ist.` : `Neue Version gefunden: ${APP_VERSION} -> ${nextStatus.remoteVersion}.`;
    ...
    function bindUiHandlers() {
      ...
      if (createForm instanceof HTMLFormElement) {
        ...
        const handleDraftInputChange = (event) => {
          ...
          const fieldName = target instanceof HTMLInputElement || target instanceof HTMLSelectElement || target instanceof HTMLTextAreaElement ? normalizeText(target.name || "") : "";
    ...
    function syncCreateFormDependencies(form) {
      ...
      bullModeSelect.title = disableBullMode ? "Bull-Modus ist bei Bull-off = Off ohne Wirkung und daher schreibgeschützt." : "";
    ...
    function handleExportFile() {
      ...
      anchor.href = url;
      anchor.download = `ata-export-${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}.json`;
      document.body.appendChild(anchor);
      anchor.click();
      anchor.remove();
      URL.revokeObjectURL(url);
      setNotice("success", "JSON-Datei exportiert.", 2e3);
    }
    async function handleExportClipboard() {
      try {
        ...
        setNotice("success", "JSON in Zwischenablage kopiert.", 2e3);
      } catch (error) {
