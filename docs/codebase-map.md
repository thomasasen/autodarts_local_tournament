...
- `src/ui`: Rendering und Bedienlogik sind getrennt. Die Render-Dateien liefern HTML pro Tab, `handlers.js` ist die Interaktions- und Orchestrierungsschicht.
- `src/bracket`: das Third-Party-Bracket-Rendering ist absichtlich gekapselt, weil es in einem isolierten `iframe` mit eigener Message-Bridge und HTML-Fallback läuft.
- `src/runtime`: Initialisierung, Observer, Route-Hooks, Public API und Cleanup liegen zentral, weil sie den Lebenszyklus des kompletten Userscripts steuern.
...
2. `scripts/build.ps1` liest diese Datei, lädt jedes Modul, entfernt alte Split-Marker und fügt die Inhalte zusammen.
3. Dasselbe Skript bettet `src/ui/styles/main.css` und `assets/pdc_logo.png` direkt ins Bundle ein.
4. Das Ergebnis landet als einzige auslieferbare Datei in `dist/autodarts-tournament-assistant.user.js`.
5. Der Loader in `installer/Autodarts Tournament Assistant Loader.user.js` lädt diese veröffentlichte Dist-Datei remote und nutzt bei Bedarf einen Cache-Fallback.
...
2. `src/data/storage.js` lädt und migriert den gespeicherten Zustand.
3. `src/ui/handlers.js` erzeugt Host und Shell im Shadow DOM.
4. `src/runtime/lifecycle.js` und `src/infra/route-hooks.js` beobachten DOM und SPA-Routen.
5. `src/infra/api-automation.js` arbeitet bei aktiviertem Feature-Flag mit `src/infra/api-client.js` gegen die Autodarts-API.
6. `src/ui/render-view.js` stößt für KO-Ansichten das Bracket-Rendering über `src/bracket/*` an.
7. `src/runtime/public-api.js` veröffentlicht `window.__ATA_RUNTIME`.
...
flowchart LR
  ...
  subgraph Runtime
    ...
    lifecycle["runtime/lifecycle.js"]
    publicapi["runtime/public-api.js"]
  end

  subgraph Data
    storage["data/storage.js"]
...
- `ui/handlers.js` ist der zentrale Knoten zwischen Benutzeraktion, Persistenz, Domain-Logik und Re-Render.
- `domain/ko-engine.js` ist der fachliche Motor für Fortschritt, Byes, Draw-Lock und abgeleitete KO-Matches.
- `runtime/lifecycle.js` verbindet Autodarts-DOM, History-Import, Autoerkennung und den lokalen Turnierzustand.
...
| `scripts/build.ps1` | Build-Orchestrierung | liest Manifest, fügt Module zusammen, bettet CSS und Logo ein, schreibt `dist/*` | `build/manifest.json`, `src/ui/styles/main.css`, `assets/pdc_logo.png`, `dist/autodarts-tournament-assistant.user.js` |
| `scripts/qa.ps1` | Gesamt-QA | ruft Build, Encoding-QA, Regelcheck-QA und Dist-Smoke-Checks auf | `scripts/build.ps1`, `scripts/qa-encoding.ps1`, `scripts/qa-regelcheck.ps1`, `dist/autodarts-tournament-assistant.user.js` |
...
| Datei | Rolle | Wichtige Inhalte / Hauptfunktionen | Primäre Verbindungen |
|---|---|---|---|
| `tests/selftest-runtime.js` | Browser-Konsole-Helfer | ruft `window.__ATA_RUNTIME.runSelfTests()` auf und formatiert das Ergebnis für `console.table` | `src/runtime/public-api.js`, `dist/autodarts-tournament-assistant.user.js` |
...
| `src/core/utils.js` | Querschnitts-Helfer | Sanitizing, HTML-Escaping, IDs, Zufall, Teilnehmer-Parsing, Routing-Key, kleine Render-Helfer | `src/data/normalization.js`, `src/domain/*`, `src/ui/*`, `src/infra/*`, `src/runtime/*` |
...
| Datei | Rolle | Wichtige Inhalte / Hauptfunktionen | Primäre Verbindungen |
|---|---|---|---|
| `src/data/storage.js` | Persistenzschicht | GM/localStorage-Lesen und -Schreiben, Store-Laden, Debounce-Persistenz, Notices | `src/data/migration.js`, `src/data/normalization.js`, `src/domain/ko-engine.js`, `src/ui/handlers.js` |
...
| Datei | Rolle | Wichtige Inhalte / Hauptfunktionen | Primäre Verbindungen |
|---|---|---|---|
| `src/domain/rules-config.js` | regelbezogene Umschalter | setzt Tie-Break-Profil und KO-Draw-Lock am aktiven Turnier, inklusive Refresh, Persist und Re-Render | `src/data/normalization.js`, `src/domain/ko-engine.js`, `src/data/storage.js`, `src/ui/handlers.js` |
| `src/domain/tournament-create.js` | Turniererzeugung und Strukturaufbau | Match-Factory, Round-Robin-Pairings, Seed- und Bye-Logik, Gruppenbildung, KO-Struktur, Validation, `createTournament` | `src/data/normalization.js`, `src/domain/ko-engine.js`, `src/ui/handlers.js`, `src/runtime/public-api.js` |
...
| `src/domain/ko-engine.js` | fachlicher KO-Motor | Winner-Advancement, Draw-Lock, Byes, KO-Meta-Snapshots, v3-Migration, Validierung und `refreshDerivedMatches` | `src/data/migration.js`, `src/data/normalization.js`, `src/domain/groups.js`, `src/domain/tournament-create.js`, `src/ui/render-view.js` |
| `src/domain/results.js` | Ergebnisvalidierung und Matchzustand | offenes Match nach Spielern finden, Legs prüfen, Sieger ableiten, Ergebnisse speichern, Editierbarkeit bestimmen | `src/domain/tournament-create.js`, `src/domain/ko-engine.js`, `src/data/storage.js`, `src/ui/handlers.js`, `src/infra/api-automation.js` |
...
| Datei | Rolle | Wichtige Inhalte / Hauptfunktionen | Primäre Verbindungen |
|---|---|---|---|
| `src/infra/api-client.js` | dünne API-Grundschicht | liest Auth-Token und Board-ID, baut Status-Bar-Infos auf, kapselt HTTP-Requests und Autodarts-Endpunkte | `src/ui/render-shell.js`, `src/infra/api-automation.js`, `src/runtime/lifecycle.js` |
| `src/infra/api-automation.js` | Matchstart und Ergebnis-Sync | erstellt Lobbys, fügt Spieler hinzu, startet Matches, synchronisiert API-Ergebnisse und löst Zuordnungsfälle auf | `src/infra/api-client.js`, `src/domain/results.js`, `src/data/storage.js`, `src/ui/handlers.js`, `src/runtime/lifecycle.js` |
| `src/infra/route-hooks.js` | SPA-Integration | patched `history.pushState` und `replaceState`, reagiert auf Routenwechsel und stößt Re-Render an | `src/core/events.js`, `src/runtime/lifecycle.js`, `src/ui/handlers.js` |
...
| `src/ui/render-tournament.js` | Turnieranlage und Turnierübersicht | rendert Neues-Turnier-Formular, PDC-Preset-Hinweise, aktives Turnier und Reset-Bereich | `src/data/normalization.js`, `src/core/utils.js`, `src/ui/handlers.js` |
| `src/ui/render-matches.js` | Matchliste und Matchaktionen | sortiert Matches, markiert `Nächstes Match`, rendert Editoren, Status und API-Start-Buttons | `src/domain/results.js`, `src/infra/api-automation.js`, `src/domain/tournament-create.js` |
...
| `src/ui/render-settings.js` | Settings-Tab | rendert Debug-Flag, API-Automation, KO-Defaults, Tie-Break-Profil und Storage-Hinweise | `src/data/normalization.js`, `src/domain/rules-config.js`, `src/ui/handlers.js` |
| `src/ui/handlers.js` | UI-Orchestrator | erstellt Host, rendert Shell, bindet Events, liest Formulare, startet Fachlogik, Import/Export und Drawer-Steuerung | `src/ui/render-shell.js`, `src/data/storage.js`, `src/domain/*`, `src/infra/api-automation.js`, `src/bracket/frame-bridge.js` |
...
| `src/bracket/frame-bridge.js` | Parent-zu-iframe-Brücke | setzt `srcdoc`, sendet Render-Payload, empfängt Height/Ready/Error-Messages und steuert Timeout/Fallback | `src/bracket/frame-template.js`, `src/bracket/payload.js`, `src/ui/render-view.js`, `src/data/storage.js` |

### Runtime

| Datei | Rolle | Wichtige Inhalte / Hauptfunktionen | Primäre Verbindungen |
|---|---|---|---|
| `src/runtime/lifecycle.js` | Laufzeitintegration in Autodarts | Autoerkennung aus DOM, History-Import, Match-Return-Shortcut, Inline-Sync, Cleanup und Event-Bridge | `src/domain/results.js`, `src/infra/api-automation.js`, `src/infra/api-client.js`, `src/ui/handlers.js` |
| `src/runtime/public-api.js` | öffentliche Runtime-API | veröffentlicht `window.__ATA_RUNTIME`, enthält `runSelfTests()` und Runtime-nahe Diagnostik | `src/runtime/lifecycle.js`, `src/domain/*`, `src/infra/api-automation.js`, Browser-Konsole |
| `src/runtime/bootstrap.js` | Startpunkt des Userscripts | lädt Store, rendert UI, installiert Hooks und Intervalle, setzt Runtime-Ready-Status | `src/data/storage.js`, `src/ui/handlers.js`, `src/runtime/lifecycle.js`, `src/infra/route-hooks.js`, `src/runtime/public-api.js` |
...
- Bei Änderungen an API- oder DOM-Erkennung zusätzlich `docs/selector-strategy.md` und die Runtime-/Infra-Dateien synchron halten.
