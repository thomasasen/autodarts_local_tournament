...
|  |  |- preliminary-schedule.js
|  |  |- preliminary-standings.js
|  |  |- preliminary-final-stage.js
|  |  |- standings-dra.js
|  |  |- groups.js
|  |  |- ko-engine.js
|  |  `- results.js
...
| `src/domain/create-validation.js` | zentrale pure Create-Validierung | analysiert Teilnehmertext und Rohwerte, vereinheitlicht Create-/Gruppen-/Vorrundenfehler, liefert stabile Reason-Codes sowie Übersicht, Matchanzahl und Prognose | `src/core/utils.js`, `src/domain/tournament-create.js`, `src/domain/preliminary-schedule.js`, `src/domain/tournament-duration.js`, `src/app/tournament-actions.js`, `src/ui/render-create-validation.js` |
...
| `src/domain/preliminary-final-stage.js` | Vorrundenqualifikation | blockiert ungeklärte Cutoff-Ties, speichert manuelle Reihenfolge und erzeugt KO/Doppel-KO aus Tabellen-Seeds | `src/domain/ko-engine.js`, `src/app/tournament-actions.js`, `src/ui/render-view.js` |
...
| `src/app/tournament-actions.js` | Turnier-Orchestrierung | validiertes Erstellen, Import, Reset, Tie-Break- und Draw-Lock-Änderungen am aktiven Turnier | `src/domain/create-validation.js`, `src/domain/tournament-create.js`, `src/domain/rules-config.js`, `src/app/derived-state.js`, `src/ui/handlers.js` |
...
| `src/app/diagnostics.js` | Runtime-Diagnostik | `runSelfTests()` für Browser-Konsole und Contract-Test, einschließlich Modussichtbarkeit, Draft-Rückkehr, Spielregel-Disclosure, Navigation, Labels, Live-Regionen, Fokusgrenzen/-Rückgabe, Re-Render-Erhalt und stabiler Turnieranlage-Hooks | `src/domain/*`, `src/infra/*`, `src/ui/*`, `src/app/public-api.js`, `scripts/test-runtime-contract.ps1` |
...
| Datei | Rolle | Wichtige Inhalte / Hauptfunktionen | Primäre Verbindungen |
|---|---|---|---|
| `src/ui/render-shell.js` | äußerer Drawer-Rahmen | rendert modalen Shadow-DOM-Drawer, natives Navigations-Landmark mit `aria-current`, typgerechte Notice-Live-Semantik und Runtime-Status-Bar | `src/ui/render-tabs.js`, `src/infra/api-client.js`, `src/ui/handlers.js` |
...
| `src/ui/create-validation-state.js` | flüchtiger Validierungszustand | verwaltet Touched-, Reveal- und Submit-Status ohne Persistenz und entscheidet, welche feldnahen Meldungen sichtbar werden | `src/ui/render-create-validation.js`, `src/ui/handlers.js`, `src/app/tournament-actions.js` |
| `src/ui/render-create-validation.js` | gezielte Validierungs-UI | aktualisiert Inline-Fehler, ARIA, Teilnehmerstatus, erweiterte Übersicht, Submit-Sperre, Fehlerzusammenfassung und Fokusziel | `src/domain/create-validation.js`, `src/ui/create-validation-state.js`, `src/ui/render-tournament.js`, `src/ui/handlers.js` |
| `src/ui/render-tournament.js` | Turnieranlage und Turnierübersicht | rendert die fünf Bereiche mit modusspezifischen Zusatzregelgruppen, Presets, Spielregel-Editor, Regelhilfe, zentralen Validierungsankern, erweiterter Live-Übersicht, aktivem Turnier und Reset-Bereich | `src/data/normalization.js`, `src/domain/create-validation.js`, `src/domain/tournament-duration.js`, `src/ui/render-create-validation.js`, `src/ui/render-tournament-help.js`, `src/ui/handlers.js` |
